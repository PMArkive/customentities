...
void NextBotGroundLocomotion::Reset( void )
{
	...
	m_nextBot = static_cast< INextBot * >( GetBot() );
...
void NextBotGroundLocomotion::Update( void )
{
	...
	m_nextBot->GetEntity()->SetAbsVelocity( m_velocity );
...
bool NextBotGroundLocomotion::DetectCollision( trace_t *pTrace, int &recursionLimit, const Vector &from, const Vector &to, const Vector &vecMins, const Vector &vecMaxs )
{
	...
	if ( them && them->ShouldTouch( m_nextBot->GetEntity() ) )
	{
		/// @todo construct mirror of trace
		them->OnContact( m_nextBot->GetEntity() );
	}
	else
	{
...
void NextBotGroundLocomotion::UpdatePosition( const Vector &newPos )
{
	...
	if ( NextBotStop.GetBool() || (m_nextBot->GetEntity()->GetFlags() & FL_FROZEN) != 0 || newPos == m_nextBot->GetPosition() )
	{
		return;
	}
...
void NextBotGroundLocomotion::UpdateGroundConstraint( void )
{
	...
	trace_t ground;
	NextBotTraceFilterIgnoreActors filter( m_nextBot->GetEntity(), body->GetCollisionGroup() );
	...
	if ( ground.fraction < 1.0f )
	{
		...
		{
			// just landed
			m_nextBot->GetEntity()->SetGroundEntity( ground.m_pEnt );
	...
	else
	{
		...
		if ( IsOnGround() )
		{
			GetBot()->OnLeaveGround( m_nextBot->GetEntity()->GetGroundEntity() );
...
{
	return (m_nextBot->GetEntity()->GetGroundEntity() != NULL);
}
...
{
	m_nextBot->GetEntity()->SetGroundEntity( NULL );
	m_ground = NULL;
...
void NextBotGroundLocomotion::DescendLadder( const CNavLadder *ladder, const CNavArea *dismountGoal )
{
	...
	if ( body )
	{
		...
		QAngle angles = m_nextBot->GetEntity()->GetLocalAngles();
		angles.y = ladderYaw;
		
		m_nextBot->GetEntity()->SetLocalAngles( angles );
...
{
	return IsClimbingOrJumping() && (m_nextBot->GetEntity()->GetAbsVelocity().z > 0.0f);
}
